...
function validateDirectorySearch(search: Record<string, unknown>): DirectorySearch {
  ...
  const limit = typeof search.limit === 'number' && Number.isInteger(search.limit)
    ? search.limit
    : 25
    ...
    <main className="mx-auto max-w-5xl p-6">
      ...
      {!response.ok ? (
        ...
        <div className="mt-8 rounded-lg border border-slate-200 bg-slate-50 p-8 text-center">
          <h2 className="text-lg font-semibold text-slate-900">No directory entries found</h2>
            ...
            <table className="min-w-full divide-y divide-slate-200 text-left text-sm">
              <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                <tr>
                  ...
                  <th className="px-5 py-3 font-semibold">Position or title</th>
